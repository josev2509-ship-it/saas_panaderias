...
class FacturaVenta(O2CBase):
    ...
    numero=models.CharField(max_length=30);cliente=models.ForeignKey("comercial.Cliente",on_delete=models.PROTECT,related_name="facturas_venta");pedido=models.ForeignKey("comercial.Pedido",on_delete=models.PROTECT,null=True,blank=True,related_name="facturas_venta");entrega=models.ForeignKey(EntregaComercial,on_delete=models.PROTECT,null=True,blank=True,related_name="facturas");moneda=models.ForeignKey("catalogos.MonedaEmpresa",on_delete=models.PROTECT);fecha=models.DateField();vence_el=models.DateField();ncf=models.CharField(max_length=30,blank=True);estado=models.CharField(max_length=25,choices=ESTADOS,default="BORRADOR");subtotal=models.DecimalField(max_digits=18,decimal_places=2,default=0);descuento=models.DecimalField(max_digits=18,decimal_places=2,default=0);impuesto=models.DecimalField(max_digits=18,decimal_places=2,default=0);total=models.DecimalField(max_digits=18,decimal_places=2,default=0);legacy_id=models.PositiveBigIntegerField(null=True,blank=True)
...
class ReciboCobro(O2CBase):
    ...
    numero=models.CharField(max_length=30);cliente=models.ForeignKey("comercial.Cliente",on_delete=models.PROTECT,related_name="recibos_cobro");moneda=models.ForeignKey("catalogos.MonedaEmpresa",on_delete=models.PROTECT);fecha=models.DateField();metodo=models.CharField(max_length=20,choices=METODOS);monto=models.DecimalField(max_digits=18,decimal_places=2);monto_aplicado=models.DecimalField(max_digits=18,decimal_places=2,default=0);referencia=models.CharField(max_length=100,blank=True);estado=models.CharField(max_length=25,choices=ESTADOS,default="BORRADOR")
...
class AplicacionCobro(models.Model):
    recibo=models.ForeignKey(ReciboCobro,on_delete=models.PROTECT,related_name="aplicaciones");cuenta=models.ForeignKey(CuentaPorCobrar,on_delete=models.PROTECT,related_name="aplicaciones");monto=models.DecimalField(max_digits=18,decimal_places=2);revertida=models.BooleanField(default=False);fecha=models.DateTimeField(auto_now_add=True)
