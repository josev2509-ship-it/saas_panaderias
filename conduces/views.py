...
def descargar_plantilla_centros(request):
    ...
    ws.append([
        "codigo", "nombre", "director", "telefono",
        "direccion", "provincia", "regional_distrito", "matricula",
    ])
...
def cargar_centros_excel(request):
    if request.method == "POST":
        ...
        if not archivo:
            ...
            return redirect("/carga-centros/")

        wb = load_workbook(archivo)
        ws = wb.active

        for indice, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=1):
            codigo, nombre, director, telefono, direccion, provincia, regional_distrito, matricula = row
...
def generar_conduces_automaticos(request):
    ...
    if request.method == "POST":
        ...
        if not empresa_id or not fecha_desde or not fecha_hasta:
            ...
            return redirect("/generar-conduces/")
        ...
        menus = MenuDiario.objects.filter(fecha__range=[fecha_desde, fecha_hasta]).order_by("fecha")
...
def buscar_conduces(request):
    codigo_centro = request.GET.get("codigo_centro", "").strip()
    ...
    if codigo_centro:
        conduces = conduces.filter(centro__codigo__icontains=codigo_centro)
...
def acciones_conduces(request):
    if request.method == "POST":
        ...
        if not ids:
            ...
            return redirect("/buscar-conduces/")
...
def editar_conduce(request, conduce_id):
    ...
    if request.method == "POST":
        conduce.numero = request.POST.get("numero", conduce.numero)
        conduce.producto = request.POST.get("producto", conduce.producto)
        conduce.cantidad = int(request.POST.get("cantidad", conduce.cantidad) or 0)
        conduce.observaciones = request.POST.get("observaciones", conduce.observaciones)
        conduce.save()

        messages.success(request, "Conduce actualizado correctamente.")
        return redirect(f"/conduce/{conduce.id}/vista/")

    return render(request, "editar_conduce.html", {"conduce": conduce})
