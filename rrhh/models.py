...
class ContratoEmpleado(Base):
 empleado=models.ForeignKey(Empleado,on_delete=models.PROTECT,related_name="contratos");numero=models.CharField(max_length=40,blank=True);version=models.PositiveIntegerField(default=1);tipo=models.CharField(max_length=30);inicio=models.DateField();fin=models.DateField(null=True,blank=True);salario=models.DecimalField(max_digits=18,decimal_places=2);estado=models.CharField(max_length=15,default="BORRADOR");puesto=models.ForeignKey(Puesto,on_delete=models.PROTECT,null=True,blank=True);jornada=models.CharField(max_length=80,blank=True);renovacion_de=models.ForeignKey("self",on_delete=models.PROTECT,null=True,blank=True,related_name="renovaciones");objeto=models.TextField(blank=True);lugar_prestacion=models.TextField(blank=True);motivo_temporal=models.TextField(blank=True);anexos=models.TextField(blank=True);observaciones=models.TextField(blank=True);firmado=models.BooleanField(default=False);firmado_en=models.DateTimeField(null=True,blank=True);firmado_por=models.ForeignKey(settings.AUTH_USER_MODEL,on_delete=models.PROTECT,null=True,blank=True,related_name="contratos_rrhh_firmados");texto_firmado=models.TextField(blank=True)
 ...
 def save(self,*args,**kwargs):
  if self.pk:
   ...
   if anterior and anterior.firmado and any(getattr(anterior,campo)!=getattr(self,campo) for campo in ("tipo","inicio","fin","salario","puesto_id","jornada","objeto","lugar_prestacion","motivo_temporal","anexos")):raise ValueError("Un contrato firmado es inmutable; registre una adenda o nueva versión.")
