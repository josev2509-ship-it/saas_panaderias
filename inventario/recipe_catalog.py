...
def evaluar_ingrediente(*, empresa, nombre, unidad):
    ...
    if match.producto_id:
        ...
        if producto and producto.unidad_medida == unidad:
            return match
        return ResultadoMatching(nombre, "REVISAR", candidatos=[{
            "id": match.producto_id, "nombre": match.producto_nombre,
        }])
    if match.estado == "SIN_COINCIDENCIA" and nombre and len(nombre) <= 180 and unidad in UNIDADES_PROVISIONALES:
    ...
    if match.estado == "REVISAR" and calificadores and nombre and unidad in UNIDADES_PROVISIONALES:
...
def resolver_ingrediente_aprobado(*, empresa, nombre, unidad):
    ...
    if len(iguales) > 1 or (iguales and iguales[0].unidad_medida != unidad):
    ...
    return ProductoInventario.objects.create(
        empresa=empresa, codigo=None, nombre=nombre, tipo="materia_prima",
