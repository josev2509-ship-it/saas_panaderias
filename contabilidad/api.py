...
def contabilizar_nota_debito_proveedor(*, context, nota, simular=False): return _regla(context, "NOTA_DEBITO_PROVEEDOR", nota, nota.monto, f"Nota débito proveedor {nota.numero}", simular=simular)
def contabilizar_aplicacion_nota_credito(*,context,aplicacion,simular=False): return _regla(context,"APLICACION_NOTA_CREDITO",aplicacion,aplicacion.monto,f"Aplicación nota crédito {aplicacion.pk}",simular=simular)
def contabilizar_aplicacion_nota_debito(*,context,aplicacion,simular=False): return _regla(context,"APLICACION_NOTA_DEBITO",aplicacion,aplicacion.monto,f"Aplicación nota débito {aplicacion.pk}",simular=simular)
...
def contabilizar_compensacion_p2p(*,context,compensacion,simular=False): return _regla(context,"COMPENSACION_P2P",compensacion,compensacion.monto_base,f"Compensacion P2P {compensacion.numero}",simular=simular)
def contabilizar_retencion_proveedor(*,context,retencion,simular=False): return _regla(context,"RETENCION_PROVEEDOR",retencion,retencion.monto,f"Retencion proveedor {retencion.numero}",simular=simular)
def revertir_compensacion_p2p(*,context,compensacion,motivo): return revertir_documento(context=context,origen_tipo="COMPENSACION_P2P",origen_id=compensacion.pk,motivo=motivo)
def revertir_retencion_proveedor(*,context,retencion,motivo): return revertir_documento(context=context,origen_tipo="RETENCION_PROVEEDOR",origen_id=retencion.pk,motivo=motivo)
...
def contabilizar_recepcion_compra(*,context,recepcion,simular=False):
    monto=sum((x.cantidad_aceptada*x.detalle_orden.precio_unitario for x in recepcion.detalles.select_related("detalle_orden")),Decimal(0));return _regla(context,"INVENTARIO",recepcion,monto,f"Recepción de compra {recepcion.numero}",simular=simular)
def contabilizar_devolucion_compra(*,context,devolucion,simular=False):
    monto=sum((x.cantidad*x.detalle_recepcion.detalle_orden.precio_unitario for x in devolucion.detalles.select_related("detalle_recepcion__detalle_orden")),Decimal(0));return _regla(context,"DEVOLUCION_COMPRA",devolucion,monto,f"Devolución de compra {devolucion.numero}",simular=simular)
