...
class AdministrativoE2E(TestCase):
 ...
 def test_e2e_nomina(self):
  d=Departamento.objects.create(empresa=self.e,codigo="ADM",nombre="Administración");p=Puesto.objects.create(empresa=self.e,codigo="A",nombre="Analista");c=CentroTrabajo.objects.create(empresa=self.e,codigo="HQ",nombre="Principal");Empleado.objects.create(empresa=self.e,codigo="E1",nombres="Ana",apellidos="Pérez",identificacion="001",puesto=p,departamento=d,centro=c,fecha_ingreso=date(2025,1,1),salario=30000);t=TipoNomina.objects.create(empresa=self.e,nombre="Mensual",periodicidad="MENSUAL");per=PeriodoNomina.objects.create(empresa=self.e,tipo=t,desde=date(2026,8,1),hasta=date(2026,8,31));ConceptoNomina.objects.create(empresa=self.e,codigo="SALARIO",nombre="Salario",tipo="INGRESO");n=procesar_nomina(empresa=self.e,periodo=per,usuario=self.u);self.assertEqual(n.total_neto,30000)
