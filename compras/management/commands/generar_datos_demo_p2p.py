...
class Command(BaseCommand):
    ...
    def _generate(self,empresa):
        ...
        invoices=[]
        for i in range(150):
            f,_=FacturaProveedor.objects.get_or_create(empresa=empresa,proveedor=orders[i%120].proveedor,numero=f"DMP2P-FAC-{i+1:04d}",defaults={"orden":orders[i%120],"recepcion":receipts[i],"ncf":f"B01{i+1:08d}","fecha":today-timedelta(days=i%45),"vence_el":today+timedelta(days=30-i%60),"moneda":mon,"subtotal":Decimal("1000")+i,"impuesto":Decimal("180"),"total":Decimal("1180")+i,"estado":["VALIDADA","PARCIALMENTE_PAGADA","VENCIDA"][i%3],"dimensiones":{"dgii_tipo_bienes_servicios":"09","dgii_servicios":str(Decimal("1000")+i),"dgii_forma_pago":"07"},"creado_por":user});invoices.append(f)
